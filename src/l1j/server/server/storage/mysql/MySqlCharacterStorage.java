...
public class MySqlCharacterStorage implements CharacterStorage {
	...
	public void createCharacter(L1PcInstance pc) {
		...
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
			//TODO 新增birthday=?,DB讀取欄位by99團隊
					.prepareStatement("INSERT INTO characters SET account_name=?,objid=?,char_name=?,birthday=?,level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,Con=?,Dex=?,Cha=?,Intel=?,Wis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,PKcount=?,PkCountForElf=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,Pay=?,HellTime=?,Banned=?,Karma=?,metempsychosis=?,LastPk=?,LastPkForElf=?,DeleteTime=?");
			//TODO 新增birthday=?,DB讀取欄位by99團隊
			...
			pstm.setInt(++i, pc.get_metempsychosis());//TODO 記錄轉生次數
	...
	public void storeCharacter(L1PcInstance pc) {
		...
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
			//TODO DB新增metempsychosis欄位分別上下各兩段
			.prepareStatement("UPDATE characters SET level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,Con=?,Dex=?,Cha=?,Intel=?,Wis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,PKcount=?,PkCountForElf=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,HellTime=?,Banned=?,Karma=?,metempsychosis=?,LastPk=?,LastPkForElf=?,DeleteTime=?,LastActive=?,AinZone=?,AinPoint=? WHERE objid=?");
			//TODO DB新增metempsychosis欄位分別上下各兩段
			pstm.setInt(++i, pc.getLevel());
			pstm.setInt(++i, pc.getHighLevel());
			pstm.setInt(++i, pc.getExp());
