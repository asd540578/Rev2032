...
public class C_NPCAction extends ClientBasePacket {
	...
	public C_NPCAction(byte abyte0[], ClientThread client) throws Exception {
		...
		else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 81289) {
			if (s.equalsIgnoreCase("a")) {
		...
		else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 70517) {
		...
		else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 81288) {
		...
		else if (((L1NpcInstance) obj).getNpcTemplate().get_npcId() == 81274) {//TODO 新手導師
			int quest_step = pc.getQuest().get_step(L1Quest.QUEST_TUTOR);//TODO 任務編號階段
			int level = pc.getLevel();//TODO 角色等級
			char s1 = s.charAt(0);
			if (level < 13) {
				switch (s1) {
