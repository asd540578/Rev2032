...
public class C_Chat extends ClientBasePacket {
	...
	public C_Chat(byte abyte0[], ClientThread clientthread) {
		...
		if (chatType == 0) { // 通常チャット
			...
			if (Config.GUI) {
				l1j.eric.gui.J_Main.getInstance().addNormalChat(pc.getName(),chatText);
			}
			// GMコマンド
			// TODO 修正一般玩家輸入"."顯示指令錯誤套用1118版做修正
			if (chatText.startsWith(".") && pc.isGm()) {//TODO 補上GM判斷
				...
				for (L1Object visible : L1World.getInstance().getAllPlayers()) {
					if (visible instanceof L1PcInstance) {
						...
						if (GM.isGm() && pc.getId() != GM.getId()) {
							GM.sendPackets(new S_SystemMessage("\\fX" + "【一般】"+ pc.getName() + ":" + chatText));
						}
					}
				}
			}
			...
			// TODO 管理者介面byeric1300460
			if (Config.GUI) {
				l1j.eric.gui.J_Main.getInstance().addWorldChat(pc.getName(),chatText);
			}
			...
			if (pc.getClanid() != 0) { // クラン所属中
				// TODO 管理者介面byeric1300460
				if (Config.GUI) {
					l1j.eric.gui.J_Main.getInstance().addClanChat(pc.getName(),chatText);
				}
					...
					if (Config.GM_OVERHEARD4) {
						for (L1Object visible : L1World.getInstance().getAllPlayers()) {
							if (visible instanceof L1PcInstance) {
								L1PcInstance GM = (L1PcInstance) visible;
								if (GM.isGm() && pc.getId() != GM.getId()) {
									GM.sendPackets(new S_SystemMessage("\\fS"+ "【血盟】" + pc.getName() + ":"+ chatText));
								}
							}
						}
					}
			...
			if (pc.isInParty()) { // パーティー中
				//TODO 管理者介面byeric1300460
				if(Config.GUI)
					l1j.eric.gui.J_Main.getInstance().addTeamChat(pc.getName(), chatText);
				//TODO 管理者介面byeric1300460
				ChatLogTable.getInstance().storeChat(pc, null, chatText,chatType);
				...
				if (Config.GM_OVERHEARD11) {
					for (L1Object visible : L1World.getInstance().getAllPlayers()) {
						if (visible instanceof L1PcInstance) {
							L1PcInstance GM = (L1PcInstance) visible;
							if (GM.isGm() && pc.getId() != GM.getId()) {
								GM.sendPackets(new S_SystemMessage("\\fR"+ "【隊伍】" + pc.getName() + ":"+ chatText));
							}
						}
					}
				}
