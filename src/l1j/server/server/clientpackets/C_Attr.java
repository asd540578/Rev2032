...
public class C_Attr extends ClientBasePacket {
			...
			if (joinPc != null) {
				if (c == 0) { // No
					...
					if (clan != null) {
						...
						if (pc.getQuest().isEnd(L1Quest.QUEST_LEVEL45)) {lv45quest = true;
						}
						...
						if (Config.MAX_CLAN_MEMBER > 0) { maxMember = Config.MAX_CLAN_MEMBER;// Clan人数の上限の設定あり
						}

						if (joinPc.getClanid() == 0) { String clanMembersName[] = clan.getAllMembers();// クラン未加入
						...
						joinPc.sendPackets(new S_ServerMessage(95, clanName)); // \f1%0血盟に加入しました。
							...
							if (Config.CLAN_ALLIANCE) {changeClan(clientthread, pc, joinPc, maxMember);
							} else {
				...
				if (i == 217) {
					...
					for (L1War war : L1World.getInstance().getWarList()) {
						if (war.CheckClanInWar(clanName)) { // 自クランが行っている戦争を発見
							if (i == 221) {war.SurrenderWar(enemyClanName, clanName); // 降伏
							} else if (i == 222) {war.CeaseWar(enemyClanName, clanName); // 終結
							}
							break;
						}
					}
	...
	private void changeClan(ClientThread clientthread, L1PcInstance pc,
		...
		if (clan != null && oldClan != null && joinPc.isCrown() && joinPc.getId() == oldClan.getLeaderId()) {// 自分が君主
			...
			for (int cnt = 0; cnt < clanMember.length; cnt++) {
				clanMember[cnt].sendPackets(new S_ServerMessage(94, joinPc.getName())); // \f1%0が血盟の一員として受け入れられました。
			}
			...
			for (int i = 0; i < oldClanMemberName.length; i++) {
				...
				if (oldClanMember != null) { // オンライン中の旧クランメンバー
					...
					// 血盟連合に加入した君主はガーディアン
					// 君主が連れてきた血盟員は見習い
					if (oldClanMember.getId() == joinPc.getId()) {oldClanMember.setClanRank(L1Clan.CLAN_RANK_GUARDIAN);
					} else {
						oldClanMember.setClanRank(L1Clan.CLAN_RANK_PROBATION);
					}
					try {oldClanMember.save();// DBにキャラクター情報を書き込む
	...
	private void callClan(L1PcInstance pc) {
		L1PcInstance callClanPc = (L1PcInstance) L1World.getInstance().findObject(pc.getTempID());pc.setTempID(0);
				if (callClanPc == null) {
					return;
				}
				...
				if (castleId != 0) {
					isInWarArea = true;
					if (WarTimeController.getInstance().isNowWar(castleId)) {isInWarArea = false; // 戦争時間中は旗内でも使用可能
					}
				}
