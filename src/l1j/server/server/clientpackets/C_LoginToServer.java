...
public class C_LoginToServer extends ClientBasePacket {
	...
	public C_LoginToServer(byte abyte0[], ClientThread client)
		...
		if (pc == null || !login.equals(pc.getAccountName())) {
			_log.info("【無法進入】 角色名稱：" + charName + " 玩家帳號：" + login + " 玩家IP："+ client.getHostname());
			client.close();
			return;
		}
		...
		if (Config.LEVEL_DOWN_RANGE != 0) {
			if (pc.getHighLevel() - pc.getLevel() >= Config.LEVEL_DOWN_RANGE) {
				_log.info("不容許的等級要求範圍的角色: 角色名稱=" + charName + " 玩家帳號=" + login+ " 玩家IP=" + client.getHostname());
				client.kick();
				return;
			}
		}
		...
		_log.info("【進入遊戲】 角色名稱：" + charName + " 玩家帳號：" + login + " 玩家IP："+ client.getHostname());
