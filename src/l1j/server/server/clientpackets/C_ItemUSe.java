...
public class C_ItemUSe extends ClientBasePacket {
	...
	public C_ItemUSe(byte abyte0[], ClientThread client) throws Exception {
		...
		if (itemId == 40088 || itemId == 40096 || itemId == 140088) {
			s = readS();
				...
				|| itemId == 49311//TODO 象牙塔對盔甲施法卷軸byattendant
				|| itemId == 49312//TODO 象牙塔對武器施法的卷軸byattendant
				|| itemId == 49290//TODO 噬血武器強化卷軸
				|| itemId == 49291//TODO 冥魔武器強化卷軸
				|| itemId == 49292//TODO 火焰武器強化卷軸
				|| itemId == 49293//TODO 光爆術武器強化卷軸
				|| itemId == 49148) {//TODO 飾品強化卷軸
			l = readD();
			...
			cookStatus = readC();
			cookNo = readC();
		} else if (itemId == 41293) { //TODO 變更為魔法釣竿by99團隊
			fishX = readH();
			fishY = readH();
		} else {
			l = readC();
		...
		if (pc.getCurrentHp() > 0) {
			...
			if (itemId == 40077 || itemId == L1ItemId.SCROLL_OF_ENCHANT_WEAPON
					...
					|| itemId == L1ItemId.B_SCROLL_OF_ENCHANT_WEAPON
					|| itemId == L1ItemId.C_SCROLL_OF_ENCHANT_WEAPON
					|| itemId == 40128|| itemId == 49312) { // 武器強化スクロール補上象牙塔對武器施法的卷軸
				...
				if (weaponId == 35 || weaponId == 48 || weaponId >= 73 
						&& weaponId < 105 && weaponId < 120 && weaponId < 147 && weaponId < 156 
						&& weaponId < 174 && weaponId < 175 && weaponId < 224 && weaponId < 7) { // イリュージョン武器
				...
				if (l1iteminstance1.getItem().getType2() != 2) {
					...
					case 5:
						if (chance < 15) {
							...
							if (l1iteminstance1.isEquipped()) {pc.addMaxHp(2);
							}
						} else if (chance > 15 && chance < 25) {
							l1iteminstance1.setaddMp(l1iteminstance1.getaddMp() + 1);
							l1iteminstance1.setaddSp(l1iteminstance1.getItem().get_addsp() + 1);
							if (l1iteminstance1.isEquipped()) {pc.addMaxMp(1);
							}
						} else if (chance > 25 && chance < 35) {
							l1iteminstance1.setFireMr(l1iteminstance1.getFireMr() + 1);
							l1iteminstance1.setWaterMr(l1iteminstance1.getWaterMr() + 1);
							...
							l1iteminstance1.setWindMr(l1iteminstance1.getWindMr() + 1);
							l1iteminstance1
							.setMpr(l1iteminstance1.getMpr() + 1);l1iteminstance1
					...
					CharactersItemStorage storage = CharactersItemStorage.create();//TODO 儲存道具
				...
				if (Config.ATTR_ENCHANT_CHANC >= _random.nextInt(100) + 1) {
					...
					pc.getInventory().updateItem(l1iteminstance1,L1PcInventory.COL_SKILLTYPE);
					pc.getInventory().saveItem(l1iteminstance1,L1PcInventory.COL_SKILLTYPE);
				} else {
					pc.sendPackets(new S_SystemMessage("武器受到強大的魔力保護幸好沒有銷毀。"));
				}
				pc.sendPackets(new S_ItemStatus(l1iteminstance1));//TODO 線上更新
				CharactersItemStorage storage = CharactersItemStorage.create();//TODO 儲存道具
				storage.updateSkillType(l1iteminstance1);//TODO 儲存道具
				pc.getInventory().removeItem(l1iteminstance, 1);//TODO 刪除道具
				//TODO 魔法武器卷軸改寫
				...
				if (itemId == 41429 && oldAttrEnchantKind == 8
						...
						|| itemId == 41432 && oldAttrEnchantKind == 2) { // 同じ屬性
					isSameAttr = true;
				}
				if (isSameAttr && oldAttrEnchantLevel >= 3) {
					pc.sendPackets(new S_ServerMessage(79)); // \f1沒有任何事情發生。//TODO 屬性卷軸強化訊息錯誤修正
					return;
				}
				...
				pc.getInventory().removeItem(l1iteminstance, 1);	
					...
					|| itemId == L1ItemId.B_SCROLL_OF_ENCHANT_ARMOR
					|| itemId == L1ItemId.C_SCROLL_OF_ENCHANT_ARMOR
					|| itemId == 40127|| itemId == 49311) { // 防具強化スクロール補上象牙塔對盔甲施法的卷軸byattendant
				if (l1iteminstance1 == null
				...
				if ((armorId == 20126) || (armorId == 20173) || (armorId == 20206) || (armorId == 20232)
						|| (armorId == 21138) || (armorId == 20028)) { // イリュージョン防具
				...
				if (l1iteminstance.getItem().getType() == 0) { // アロー
					...
					if (pc.getBaseStr() < Config.BONUS_STATS3
							&& pc.getElixirStats() < Config.BONUS_STATS2) { // TODO 調整能力值上限
						...
						pc.sendPackets(new S_SystemMessage("萬能藥力量已達到上限請選擇其他能力值。"));
					}
					...
					if (pc.getBaseCon() < Config.BONUS_STATS3
						...
						pc.sendPackets(new S_SystemMessage("萬能藥體力已達到上限請選擇其他能力值。"));
					}
					...
					if (pc.getBaseDex() < Config.BONUS_STATS3
						...
						pc.sendPackets(new S_SystemMessage("萬能藥敏捷已達到上限請選擇其他能力值。"));
					}
					...
					if (pc.getBaseInt() < Config.BONUS_STATS3
						...
						pc.sendPackets(new S_SystemMessage("萬能藥智慧已達到上限請選擇其他能力值。"));
					}
					...
					if (pc.getBaseWis() < Config.BONUS_STATS3
						...
						pc.sendPackets(new S_SystemMessage("萬能藥精神已達到上限請選擇其他能力值。"));
					}
					...
					if (pc.getBaseCha() < Config.BONUS_STATS3
						...
						pc.sendPackets(new S_SystemMessage("萬能藥魅力已達到上限請選擇其他能力值。"));
					}
					...
					L1Teleport.teleport(pc, ((L1EtcItem) l1iteminstance.getItem()).get_locx(),
							((L1EtcItem) l1iteminstance.getItem()).get_locy(),
							((L1EtcItem) l1iteminstance.getItem()).get_mapid(), 5, true);
					BroadCastToAll("\\fV" + "玩家【" + pc.getName()+ "】在極大的痛苦中感到精神甦醒。");
					pc.getInventory().removeItem(l1iteminstance, 1);
				...
				// レッドポーション、濃縮體力回復劑、象牙の塔の體力回復劑
				else if (itemId == L1ItemId.POTION_OF_HEALING
						|| itemId == L1ItemId.CONDENSED_POTION_OF_HEALING || itemId == 40029) {
					UseHeallingPotion(pc, 15, 189);
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == 240112) { //TODO 情人的祝福
					if (pc.getInventory().consumeItem(40308,2800)) {//TODO 要扣除的金幣數量
						pc.setCurrentHp(pc.getCurrentHp() + 100);//TODO 補血的量
						pc.setCurrentMp(pc.getCurrentMp() + 100);//TODO 補魔的量
						pc.sendPackets(new S_SkillSound(pc.getId(), 189));
						pc.broadcastPacket(new S_SkillSound(pc.getId(), 189));
					} else { 
						pc.sendPackets(new S_SystemMessage("金幣不足。"));
					}
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == 40022) { // 古代の體力回復劑
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == 40023) { // 古代の高級體力回復劑
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == 40024) { // 古代の強力體力回復劑
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == L1ItemId.B_POTION_OF_EXTRA_HEALING) { // 祝福されたオレンジ ポーション
					UseHeallingPotion(pc, 55, 194);
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == L1ItemId.B_POTION_OF_GREATER_HEALING) { // 祝福されたクリアーポーション
					...
					pc.getInventory().removeItem(l1iteminstance, 1);
				} else if (itemId == 40733) { //TODO 名譽貨幣
					if (!pc.isDragonKnight() && !pc.isIllusionist()) { //TODO 龍騎士與幻術師無法使用
						useBravePotion(pc, itemId);
					} else {
					...
					if (!l1iteminstance1.isIdentified()) {l1iteminstance1.setIdentified(true);
					...
					if (itemId == 49281) { // フィジカルエンチャント：STR
						...
						skillid = 49;
					} else if (itemId == 49284) { // ホーリーウォーク(未實裝)
						skillid = 52;
						return;
					 ...
					 * 40382 // 地圖各種 || itemId >= 40385 && itemId <= 40390) {
					 */
					if (itemId >= 40373 && itemId <= 40390) {
						...
						for (Object petObject : petList) {
							if (petObject instanceof L1PetInstance) { // ペット
								L1PetInstance pet = (L1PetInstance) petObject;pet.call();
							}
						}
								...
								if (L1World.getInstance()
									...
									for (L1PcInstance visiblePc : L1World
											.getInstance().getVisiblePlayer(target,
													0)) {
										if (!visiblePc.isDead()) {// \f1その場所に他の人が立っているので復活させることができません。
						...
						if (chance >= 0 && chance < 5) {UseHeallingPotion(pc, 15, 189);
						} else if (chance >= 5 && chance < 9) {
					 ...
					 * else if (itemId == 40101) { // 隱された溪谷掃還スクロール
					 * pc.sendPackets(new S_NPCTalkReturn(pc.getId(), "ei037")); }
					 */else if (itemId == 41209) { // ポピレアの依賴書 TODO 修正隱藏卷無法使用
	...
	private void useBravePotion(L1PcInstance pc, int item_id) {
		...
		if (item_id == L1ItemId.POTION_OF_EMOTION_BRAVERY) { // ブレイブ ポーション
		...
		} else if (item_id == 40039) { //TODO 紅酒
			time = 600;
			if (pc.hasSkillEffect(HOLY_WALK)) { //TODO 修正紅酒與疾走加速重複問題
				pc.killSkillEffectTimer(HOLY_WALK);
				pc.sendPackets(new S_SkillBrave(pc.getId(), 0, 0));
				pc.broadcastPacket(new S_SkillBrave(pc.getId(), 0, 0));
				pc.setBraveSpeed(0);
			}
		} else if (item_id == 40040) { //TODO 威士忌
			time = 900;
			if (pc.hasSkillEffect(MOVING_ACCELERATION)) { //TODO 修正威士忌與行走加速重複問題
			...
			if (pc.hasSkillEffect(STATUS_RIBRAVE)) { // ユグドラの實とは重複しない
				pc.killSkillEffectTimer(STATUS_RIBRAVE);
				// XXX ユグドラの實のアイコンを消す方法が不明
				pc.setBraveSpeed(0);
			}
	...
	private boolean usePolyScroll(L1PcInstance pc, int item_id, String s) {
		...
		if (awakeSkillId == AWAKEN_ANTHARAS || awakeSkillId == AWAKEN_FAFURION
			...
			pc.sendPackets(new S_ServerMessage(1384)); // 現在の狀態では變身できません。
			return false;
		}
	...
	private void usePolyScale(L1PcInstance pc, int itemId) {
		...
		if (itemId == 41154) { // 闇の鱗
			...
			polyId = 3784;
		} else if (itemId == 49220) { // オーク密使變身スクロール
			polyId = 6984;
		}
	...
	private void usePolyPotion(L1PcInstance pc, int itemId) {
		...
		int polyId = 0;
		if (itemId == 41143) { // ラバーボーンヘッド變身ポーション
			polyId = 6086;
		} else if (itemId == 41144) { // ラバーボーンアーチャー變身ポーション
			polyId = 6087;
		} else if (itemId == 41145) { // ラバーボーンナイフ變身ポーション
			polyId = 6088;
		} else if (itemId == 49149 && pc.get_sex() == 0 && pc.isCrown()) { // シャルナの變身スクロール（レベル30）
			polyId = 6822;
			...
			polyId = 7142;
		} else if (itemId == 49150 && pc.get_sex() == 0 && pc.isCrown()) { // シャルナの變身スクロール（レベル40）
			polyId = 6832;
			...
			polyId = 7146;
		} else if (itemId == 49151 && pc.get_sex() == 0 && pc.isCrown()) { // シャルナの變身スクロール（レベル52）
			polyId = 6842;
			...
			polyId = 7150;
		} else if (itemId == 49152 && pc.get_sex() == 0 && pc.isCrown()) { // シャルナの變身スクロール（レベル55）
			polyId = 6852;
			...
			polyId = 7154;
		} else if (itemId == 49153 && pc.get_sex() == 0 && pc.isCrown()) { // シャルナの變身スクロール（レベル60）
			polyId = 6862;
			...
			polyId = 7158;
		} else if (itemId == 49154 && pc.get_sex() == 0 && pc.isCrown()) { // シャルナの變身スクロール（レベル65）
			polyId = 6872;
			...
			polyId = 7162;
		} else if (itemId == 49155 && pc.get_sex() == 0 && pc.isCrown()) { // シャルナの變身スクロール（レベル70）
			polyId = 6882;
	...
	private void useSpellBook(L1PcInstance pc, L1ItemInstance item, int itemId) {
		...
		if (pc.isGm()) {SpellBook(pc, item, isLawful);
		...
		} else if (itemAttr != locAttr && itemAttr != 0 && locAttr != 0) {// 間違ったテンプルで讀んだ場合雷が落ちる
			pc.sendPackets(new S_ServerMessage(79)); // \f1何も起きませんでした。
	...
	private void saveLetter(int itemObjectId, int code, String sender,String receiver, byte[] text) {// 日付を取得する
	...
	private void startFishing(L1PcInstance pc, int itemId, int fishX, int fishY) {
		//TODO 修正3.3C釣竿下竿座標位置判斷by99團隊
		if (pc.getMapId() != 5300 || fishX <= 32704 || fishX >= 32831
		...
		int rodLength = 0;
		if (itemId == 41293) {//變更為魔法釣竿//TODO 變更為魔法釣竿by99團隊
			rodLength = 5;
		}
		if (pc.getMap().isFishingZone(fishX, fishY)) {
			if (pc.getMap().isFishingZone(fishX + 1, fishY)&& pc.getMap().isFishingZone(fishX - 1, fishY)
				...
				if (fishX > pc.getX() + rodLength
						|| fishX < pc.getX() - rodLength) {// ここに釣り竿を投げることはできません。
					pc.sendPackets(new S_ServerMessage(1138));
				} else if (fishY > pc.getY() + rodLength
						|| fishY < pc.getY() - rodLength) {// ここに釣り竿を投げることはできません。
					pc.sendPackets(new S_ServerMessage(1138));
				} else if (pc.getInventory().consumeItem(41295, 1)) { //TODO 新鮮的餌
	...
	private void useMagicDoll(L1PcInstance pc, int itemId, int itemObjectId) {
		...
		if (magic_doll != null) {
			...
			if (isAppear) {
				...
				pc.sendPackets(new S_SkillIconGFX(56, 1800));//TODO 魔法娃娃判斷秒數
				pc.sendPackets(new S_SystemMessage("魔法娃娃會保護你。"));
				pc.sendPackets(new S_OwnCharStatus(pc));
				pc.getInventory().consumeItem(41246, 50);//TODO 魔法結晶體不足。
			} else {
	...
	private void useFurnitureItem(L1PcInstance pc, int itemId, int itemObjectId) {
		...
		for (L1Object l1object : L1World.getInstance().getObject()) {
			if (l1object instanceof L1FurnitureInstance) {
				...
				if (furniture.getItemObjId() == itemObjectId) { // 既に引き出している傢俱
					isAppear = false;
					break;
				}
			}
