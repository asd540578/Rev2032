...
public class C_Mail extends ClientBasePacket {
	...
	private static int TYPE_NORMAL_MAIL = 0;//TODO 一般
	private static int TYPE_CLAN_MAIL = 1;//TODO 血盟
	private static int TYPE_MAIL_BOX = 2;//TODO 保管箱
	...
	public C_Mail(byte abyte0[], ClientThread client) {
		...
		if (type == 0x00 || type == 0x01 || type == 0x02) {//TODO 開く
			pc.sendPackets(new S_Mail(pc.getName(), type));
		} else if (type == 0x10 || type == 0x11 || type == 0x12) {//TODO 読む
			int mailId = readD();
			...
			if (receiver != null) {//TODO オンライン中
				...
				try {
					...
					if (restorePc != null) {
						...
						pc.sendPackets(new S_ServerMessage(109, receiverName));//TODO %0という名前の人はいません。
					}
				} catch (Exception e) {
					_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
				}
			}
		} else if (type == 0x21) {//TODO 血盟メールを書く
			int unknow = readH();
			...
			if (clan != null) {
				for (String name : clan.getAllMembers()) {
					...
					if (clanPc != null) {//TODO オンライン中
						clanPc.sendPackets(new S_Mail(name, TYPE_CLAN_MAIL));
					}
				}
			}
		} else if (type == 0x30 || type == 0x31 || type == 0x32) {//TODO 削除
			int mailId = readD();
			MailTable.getInstance().deleteMail(mailId);
			pc.sendPackets(new S_Mail(mailId, type));
		} else if (type == 0x40) {//TODO 保管箱に保存
			int mailId = readD();
