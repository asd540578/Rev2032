	...
	public C_Who(byte[] decrypt, ClientThread client) {
		...
		if (find != null) {
			...
			if (Config.ALT_WHO_COMMAND) {
				...
				pc.sendPackets(new S_SystemMessage("經驗值: " + Config.RATE_XP+ " 倍"));
				pc.sendPackets(new S_SystemMessage("掉寶率: "+ Config.RATE_DROP_ITEMS + " 倍"));
				pc.sendPackets(new S_SystemMessage("取得金幣: "+ Config.RATE_DROP_ADENA + " 倍"));
				pc.sendPackets(new S_SystemMessage("友好度倍率: "+ Config.RATE_KARMA + " 倍"));
				pc.sendPackets(new S_SystemMessage("正義值倍率: "+ Config.RATE_LA + " 倍"));
				pc.sendPackets(new S_SystemMessage("衝裝率: 武器 "+ Config.ENCHANT_CHANCE_WEAPON + "%  /  防具 "+ Config.ENCHANT_CHANCE_ARMOR + "%"));
