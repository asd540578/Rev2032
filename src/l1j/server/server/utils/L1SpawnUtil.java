...
//TODO 新增怪物隨機地圖產生 by eric1300460
		...
		try {
			...
			for (int i = 0; i < mobCont; i++) {
				...
				if (tryCount >= 50) {
					boolean find = false;
					L1PcInstance findPc;
					for (Object obj : L1World.getInstance().getVisibleObjects(
							mapId).values()) {
				...
				//L1World.getInstance().broadcastServerMessage("【" + npc[i].getName() + "】出現在"+ MapsTable.getInstance().getLoaction(mapId)+ " MapID=" + mapId + " X=" + npc[i].getX()+ " Y=" + npc[i].getY());
			}
			int timeSecondToDelete = mobTable
			.getTimeSecondToDelete(randomMobId);
