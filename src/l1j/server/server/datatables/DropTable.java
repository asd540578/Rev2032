...
public class DropTable {
	...
	public void dropShare(L1NpcInstance npc, ArrayList acquisitorList,
		...
		for (int i = inventory.getSize(); i > 0; i--) {
							...
							if (acquisitor instanceof L1PcInstance) {
								...
								if (l1iteminstance != null
									...
									if (player.isInParty()) { // パーティの場合
										...
										for (int p = 0; p < partyMember.length; p++) {partyMember[p].sendPackets(new S_ServerMessage(813, npc.getName(),
			...
			if (itemId >= 40131 && itemId <= 40135) {
				if (isGround || targetInventory == null) {inventory.removeItem(item, item.getCount());
				continue;
				}
			}
