...
public class WarTimeController implements Runnable {
	...
	private void checkWarTime() {
		for (int i = 0; i < 8; i++) {
			//TODO 增加GM指令(修改攻城時間)byseronet&&eric1300460 
			int j = i;
			_l1castle[j] = CastleTable.getInstance().getCastleTable(j + 1);
			_war_start_time[j] = _l1castle[j].getWarTime();
			_war_end_time[j] = (Calendar) _l1castle[j].getWarTime().clone();
			_war_end_time[j].add(Config.ALT_WAR_TIME_UNIT, Config.ALT_WAR_TIME);
