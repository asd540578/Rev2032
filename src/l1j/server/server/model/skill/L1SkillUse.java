...
public class L1SkillUse {
	...
	private void sendIcon(L1PcInstance pc) {
		...
		} else if (_skillId == UNCANNY_DODGE ) { //TODO 暗影閃避
			pc.sendPackets(new S_PacketBox(88, pc.getDodge()));
		} else if (_skillId == MIRROR_IMAGE  ) { //TODO 鏡像
			pc.sendPackets(new S_PacketBox(88, pc.getDodge()));
			//TODO 暗影閃避byajackk2001
			...
			.hasNext();) {
				...
				// チルタッチ、バンパイアリックタッチ
					...
					int[] BowGFX = {138, 37, 3860, 3126, 3420, 2284, 3105,
							...
							6866, 6867, 6876, 6877, 6886, 6887, 8842, 8845,
							9225, 9226, 8719, 8900, 8913, 9214, 371,};
					//TODO 追加3.4新變身補上三重矢判斷
				...
				// ★★★ 確率系スキル ★★★
					...
					int[] stunTimeArray = { 1000, 1050, 1100, 1150, 1200, 1250,
							1300, 1350, 1400, 1450, 1500, 1550, 1600, 1650,
							1700, 1750, 1800, 1850, 1900, 1950, 2000 };//TODO 衝暈秒數2/3
					Random random = new Random();
					int rnd = random.nextInt(stunTimeArray.length);
					_shockStunDuration = stunTimeArray[rnd]* Config.SHOCK_STUN_TIMER;//TODO 衝暈秒數
					if (cha instanceof L1PcInstance
				...
				if (_calcType == PC_PC || _calcType == NPC_PC) {
					...
					} else if (_skillId == CALL_CLAN) { //TODO 呼喚盟友
						L1PcInstance pc = (L1PcInstance) cha;
					...
					} else if (_skillId == RUN_CLAN) { //TODO 援護盟友
						L1PcInstance pc = (L1PcInstance) cha;
						L1PcInstance clanPc = (L1PcInstance) L1World
						.getInstance().findObject(_targetID);
						if (clanPc != null) {
							if (pc.getMap().isEscapable() || pc.isGm()) {
								...
								.checkInAllWarArea(// いずれかの城エリア
										clanPc.getX(), clanPc.getY(),clanPc.getMapId());
								if ((clanPc.getMapId() == 0
										|| clanPc.getMapId() == 4 || clanPc.getMapId() == 304)
										&& castle_area == false) {
					...
					if (_skillId == LIGHT) { // ライト
						...
						L1PcInstance pc = (L1PcInstance) cha;
						pc.addAc(-20);
					} else if (_skillId == ILLUSION_AVATAR) {//TODO 幻術師技能幻覺幻身
						L1PcInstance pc = (L1PcInstance) cha;
						pc.addDmgup(10);
						pc.addBowDmgup(10);
						//TODO by0968026609
						...
						pc.addWis(1);//TODO 精神
						pc.addInt(1);//TODO 智力
					} else if (_skillId == PANIC) { {//TODO 實裝幻術師恐慌by0968026609
						L1PcInstance pc = (L1PcInstance) cha;
						pc.addStr(-1);//TODO 力量
						pc.addCon(-1);//TODO 體質
						pc.addDex(-1);//TODO 敏捷
	...
	public void handleCommands(L1PcInstance target, int element, int id, int x,
			int y, Object message, int buffDuration, int type, int typeGmbuff) {
