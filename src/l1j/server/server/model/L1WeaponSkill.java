...
public class L1WeaponSkill {
	...
	private static int _WeaponSkillType;
	...
	public static double getWeaponSkillDamage(L1PcInstance pc, L1Character cha,
		...
		if (effectId != 0) {
			...
			if (!isArrowType) {
                ...
                effectId, cha.getX(), cha.getY(),ActionCodes.ACTION_Attack, false);
	...
	//TODO 光爆武器強化卷軸魔法改寫
	public static double getDamage(L1PcInstance pc, L1Character cha) {
		double dmg = 0;
		int chance = _random.nextInt(100) + 1;
		if (30 >= chance) {
			int diceCount = 2;
			int Level = pc.getLevel() / 2;
			double bsk = 0;
			double damageRate = 1.4D;
			int LvlDmg = 0;
			bsk = 1.2;
			for (int i = 0; i < diceCount; i++) {
				LvlDmg += (_random.nextInt(Level) + 1) * (bsk + damageRate);
			}    
			dmg = LvlDmg * damageRate;
			int attr = 5;
			for (int i = 5; i > 0; i--) {//TODO 要出現幾個特效
				L1Location newLocation = cha.getLocation().randomLocation(attr, true);//TODO 周圍幾格內出現
				int getX = newLocation.getX();//TODO 取得隨機x座標
				int getY = newLocation.getY();//TODO 取得隨機y座標
				S_EffectLocation spr = new S_EffectLocation(getX, getY, 6288);
				pc.sendPackets(spr);
				pc.broadcastPacket(spr);
			}
			for (L1Object object : L1World.getInstance().getVisibleObjects(cha, attr)) {
				if (object == null) {
					continue;
				}
				if (!(object instanceof L1Character)) {
				...
				S_EffectLocation spr = new S_EffectLocation(cha.getX(), cha.getY(), 6288);
				pc.sendPackets(spr);
				pc.broadcastPacket(spr);
				if (object instanceof L1PcInstance) {
					L1PcInstance targetPc = (L1PcInstance) object;
	...
	//TODO 冰雨武器強化卷軸魔法改寫
	public static double getBaphometS(L1PcInstance pc, L1Character cha) {
		double dmg = 0;
		int chance = _random.nextInt(100) + 1;
		if (20 >= chance) {
			int diceCount = 2;
			...
			for (int i = 2; i > 0; i--) {//TODO 要出現幾個特效
				...
				int getX = newLocation.getX();//TODO 取得隨機x座標
				int getY = newLocation.getY();//TODO 取得隨機y座標
				S_EffectLocation spr = new S_EffectLocation(getX, getY, 7771);
				pc.sendPackets(spr);
				pc.broadcastPacket(spr);
			}
			for (L1Object object : L1World.getInstance().getVisibleObjects(cha, attr)) {
				...
				S_EffectLocation spr = new S_EffectLocation(cha.getX(), cha.getY(), 7771);
				pc.sendPackets(spr);
				pc.broadcastPacket(spr);
				if (object instanceof L1PcInstance) {
					L1PcInstance targetPc = (L1PcInstance) object;
