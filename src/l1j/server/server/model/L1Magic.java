...
public class L1Magic {
	...
	private int calcProbability(int skillId) {
		...
		if (_calcType == PC_PC || _calcType == PC_NPC) {
			...
			attackInt = _pc.getInt(); //加入攻擊者智力
		} else {
			attackLevel = _npc.getLevel();
			attackInt = _npc.getInt(); //加入攻擊者智力
		}
		...
		} else if (skillId == BONE_BREAK || skillId == CONFUSION //TODO 骷髏毀壞、混亂by9001183ex
				|| skillId == PANIC || skillId == PHANTASM) { //TODO 恐慌、幻想by9001183ex
			probability = l1skills.getProbabilityValue() + (attackInt / 2);
			//TODO 魔法命中by9001183ex
			if (_calcType == PC_PC || _calcType == PC_NPC) {
				probability += 2 * _pc.getOriginalMagicHit();
			}                        
		} else if (skillId == SHOCK_STUN || skillId == SHOCK_SKIN) { //TODO 衝擊之暈、衝擊之膚by9001183ex
		...
		} else if (skillId == COUNTER_BARRIER) { //TODO 反擊屏障by9001183ex
			//TODO  成功確率基本確率 + LV差1每+-2%
			probability = l1skills.getProbabilityValue() + (attackLevel - defenseLevel) * 2;
			//TODO 魔法命中by9001183ex
			if (_calcType == PC_PC || _calcType == PC_NPC) {
