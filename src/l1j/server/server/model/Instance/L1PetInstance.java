...
public class L1PetInstance extends L1NpcInstance {
	...
	public void onFinalAction(L1PcInstance player, String action) {
		...
		if (status == 6) {
			...
			L1PcInstance _petMaster = (L1PcInstance) _master;
			liberate(); // ペットの解放
			Object[] petList = _petMaster.getPetList().values().toArray();
			for (Object petObject : petList) {
				if (petObject instanceof L1SummonInstance) {
					L1SummonInstance summon = (L1SummonInstance) petObject;
					_petMaster.sendPackets(new S_SummonPack(summon, _petMaster));
					return;
				} else if (petObject instanceof L1PetInstance) {
					L1PetInstance pet = (L1PetInstance) petObject;
					_petMaster.sendPackets(new S_PetPack(pet, _petMaster));
					return;
				}
			}
			//TODO 3.5C關閉寵物圖型介面UI變更by99團隊
