...
import static l1j.server.server.model.skill.L1SkillId.HOLY_WEAPON;//TODO 新手導師
import static l1j.server.server.model.skill.L1SkillId.STATUS_HASTE;//TODO 新手導師
...
public class L1MerchantInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (talking != null) {
			if (npcid == 70841) { // ルーディエル
				...
				if (player.isDarkelf()) {
					if (player.getTempCharGfx() == 8783) { //TODO 45任務的刺客變身判斷
						int lv45_step = quest.get_step(L1Quest.QUEST_LEVEL45);
						if (lv45_step == 1) {
							htmlid = "assassin1";
						} else if (lv45_step == 2) {
				...
				//TODO 修正治療師加速師不出現空白對話
				/*
				 * } else if (npcid == 70512) { // 治療師（歌う島 村の中） if
				 * (player.getLevel() >= 25) { htmlid = "jpe0102"; } } else if
				 * (npcid == 70514) { // ヘイスト師 if (player.getLevel() >= 25) {
				 * htmlid = "jpe0092"; }
				 */
				...
				int quest_step = quest.get_step(L1Quest.QUEST_TUTOR);//TODO 任務編號階段
				int playerLv = player.getLevel();//TODO 角色等級
				if (playerLv < 13) {
					newUserHelp(player, 1);//TODO 血魔補滿
				}
				if (playerLv < 2 && quest_step == 0) {
					player.addExp(125);//TODO 給予等級一等
					L1ItemInstance item = player.getInventory().storeItem(40101, 5); //TODO 指定傳送卷軸(隱藏之谷)
					player.sendPackets(new S_ServerMessage(143, item.getItem().getName()));
					quest.set_step(L1Quest.QUEST_TUTOR, 1); //TODO 設定任務
					htmlid = "";
					...
					htmlid = "tutord";//TODO 接受幫助
				} else if (playerLv < 13 && player.isDragonKnight()) {
					htmlid = "tutordk";//TODO 接受幫助
				} else if (playerLv < 13 && player.isElf()) {
					htmlid = "tutore";//TODO 接受幫助
				} else if (playerLv < 13 && player.isIllusionist()) {
					htmlid = "tutori";//TODO 接受幫助
				} else if (playerLv < 13 && player.isKnight()) {
					htmlid = "tutork";//TODO 接受幫助
				} else if (playerLv < 13 && player.isWizard()) {
					htmlid = "tutorm";//TODO 接受幫助
				} else if (playerLv < 13 && player.isCrown()) {
					htmlid = "tutorp";//TODO 接受幫助
				} else {
					htmlid = "tutorend";
				}
				// TODO 修正釣魚小童波爾等級判斷by0968026609&BAO提供教學參考
			} else if (npcid == 80082) { //TODO 釣魚小童波爾
				int playerLv = player.getLevel();//TODO 角色等級
				if (playerLv < 15) {//TODO 等級判斷
					htmlid = "fk_in_lv";
				}
			}
			// TODO 修正釣魚小童波爾等級判斷by0968026609&BAO提供教學參考
	...
	private String talkToRuba(L1PcInstance pc) {
		...
		//TODO 修正蘿芭的對話 by 阿傑
		/*
		 * 刪除if (pc.isCrown() || pc.isWizard()) { htmlid = "en0101"; } else if
		 * (pc.isKnight() || pc.isElf() || pc.isDarkelf()) { htmlid = "en0102";
		 * }刪除
		 */
		//TODO 變更
		if (pc.isCrown() || pc.isWizard()) {//TODO 王族、法師
			htmlid = "ruba";
