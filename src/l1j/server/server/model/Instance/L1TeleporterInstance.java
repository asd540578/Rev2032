...
public class L1TeleporterInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (talking != null) {
			...
			else if (npcid == 81272) {//TODO 龍騎士 費艾娜傳送貝希摩斯by99團隊
				if (player.isDragonKnight()) {
					htmlid = "feaena1";
				} else {
					htmlid = "feaena2";
				}
			}else if (npcid == 81273) {//TODO 幻術士 艾希雅傳送希培利亞by99團隊
				if (player.isIllusionist()) {
					htmlid = "asha1";
				} else {
					htmlid = "asha2";
				}
			} else if (npcid == 71013) { //TODO 卡連傳送沉默洞穴by99團隊
				if (player.isDarkelf()) {
			...
			else if(npcid == 50055){//TODO 德瑞斯特⇒傳送新手村-隱藏之谷by87you
				if(player.getLevel()>13){
					htmlid="drist1";
				}else{
					htmlid="drist";
