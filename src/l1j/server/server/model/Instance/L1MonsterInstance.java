...
public class L1MonsterInstance extends L1NpcInstance {
	...
	public void receiveDamage(L1Character attacker, int damage) { // 攻撃でＨＰを減らすときはここを使用
		if (getCurrentHp() > 0 && !isDead()) {
			...
			if (attacker instanceof L1PcInstance && damage > 0) {
				...
				//TODO GM測試用怪物
				if (getNpcTemplate().get_npcId() == 81305 || getNpcTemplate().get_npcId() == 81306) {
	...
	private void distributeExpDropKarma(L1Character lastAttacker) {
		//TODO 怪物說話-怪死說話 use 丫傑 code 3/4
		...
		if (pc != null) {
			...
			MobTalk.forL1MonsterTalking(npc, 3);
			//TODO 怪物說話-怪物死亡說話 use 丫傑 code 4/4
	...
	private void hide() {
		int npcid = getNpcTemplate().get_npcId();
		int GfxId = getNpcTemplate().get_gfxid();//TODO 思克巴瀕死傳送延遲by9001183ex
		if (npcid == 45061 // カーズドスパルトイ
			...
			if (getMaxHp() / 3 > getCurrentHp()) {
				...
				if (1 > rnd) {
					...
					broadcastPacket(new S_DoActionGFX(getId(),ActionCodes.ACTION_Hide));setStatus(13);
					broadcastPacket(new S_NPCPack(this));
				}
			}
			//TODO 思克巴瀕死傳送延遲by9001183ex
		} else if (GfxId == 1112 //TODO 思克巴
				|| GfxId == 1116 //TODO 思克巴女皇
				|| GfxId == 4091) { //TODO 思克巴(肯特監獄)
			if (getMaxHp() / 3 > getCurrentHp() //TODO hp低於3/1
				...
				if (Config.DYINH_TRANSMISSINN >= chance) {
					L1Location newLocation = getLocation().randomLocation(3, true);
					if (nearTeleport(newLocation.getX(), newLocation.getY()) == true) {
						...
						broadcastPacket(new S_NPCPack(this)); //TODO 重新發布NPCPack避免傳送後變隱形
						return;
					}
				}
			}
