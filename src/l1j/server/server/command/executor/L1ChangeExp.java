...
public class L1ChangeExp implements L1CommandExecutor {
	private static Logger _log = Logger.getLogger(L1ChangeExp.class.getName());
			...
			if (arg1 != null && !arg1.equals("")) {
				...
				if (arg2 != null && !arg2.equals("")) {
					...
					if (this.changeExpThread == null) {
						...
						this.changeExpThread.interrupt();//TODO 先中斷原來的行程
						//TODO 在開創新的
						this.changeExpThread = new Thread() {
							@Override
							public void run() {
				...
				try {
					Thread.sleep(400); //TODO 讓前一個行程有時間400豪秒
				} catch (Exception e) {
					System.out.println(e);
				}
				L1World.getInstance().broadcastServerMessage("系統公告" + ": " + "經驗值倍率已調整為" + "【" + Config.RATE_XP+ " 】" + "倍" + "。");
				pc.sendPackets(new S_SystemMessage("經驗值倍率已調整為" + "【"+ Config.RATE_XP + " 】" + "倍" + "。"));
			} else {
				pc.sendPackets(new S_SystemMessage("目前的經驗值倍率為" + "【"+ Config.RATE_XP + " 】" + "倍" + "。"));
			}
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage("請輸入『.經驗倍率』。"));
		}
	}
}
